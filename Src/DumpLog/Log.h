...
Module Name:
    Log.h
Abstract:
    This module implements Installer logging routines that logs/traces the program events.
Author:
    Yongzhen luo (yongzhen.luo@elex-tech.com) Mar/26/2014
Note:
    None.
--*/
...
#define MODULE_NAME L"WpmService"
 ...
 * @return HRESULT: return S_OK if successful.
 */
LOG_DLL_API__ HRESULT SetLogLevel( __in LOG_LEVEL enumLogLevel);

LOG_DLL_API__ HRESULT SetModuleName(__in LPCWSTR strModuleName);
 ...
 */
LOG_DLL_API__ LOG_LEVEL GetCurLogLevel();
...
*/
LOG_DLL_API__ HRESULT GetLogFileName( __inout LPWSTR lpszFileName );
...
LOG_DLL_API__ void TraceErrorReport(__in DWORD Status, __in LPCWSTR ModuleName, __in LPCWSTR FileName, __in DWORD LineNumber );
